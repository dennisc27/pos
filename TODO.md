...
- [ ] implement the "Inventory Report" report with all the inner reports, including a title, 1–3 metrics at the top(items with more than 3 months on sale, total of damaged items, total qty), a table at the bottom and a return button. it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Marketing" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button. Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Customer Report" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button. Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Expenses" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button.it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Incomes" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button.it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Pawns Created" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button.it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Pawns Redeemed" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button.it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Pawns Forfeited" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button.it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Pawns on Sale" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button.it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Loan Book" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button.it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Products Aged" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button. Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Voids / Refund by staff who sold it" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button.it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] implement the "Shift Reports" report with all the inner reports, including a title, 1–3 metrics at the top, a table at the bottom and a return button.it should also have at the beginning a dialog that asks for the start date and end date. Keep the format for dates dd/MM/yyyy. Reuse the components as much as possible with other reports (such as the start/end date dialog, page). Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] on point of sale-> new sale: delete the button collect payment as it is redundant. Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] on point of sale-> new sale: where the button "See all inventory" is, put the button All products and remove the "see all inventory". Make sure the report uses the database structure on schema.sql, if not add it to the file (extraschema.sql)
- [ ] on point of sale-> buy: change the seller profile to match the customer change and add component that the new sale has. See picture on pics\customer.png. 
- [ ] on point of sale-> buy: change the offer panel to have just the offer to seller and it must be just an input textfield that the user will fill.
- [ ] on purchasing - receive add a supplier 
