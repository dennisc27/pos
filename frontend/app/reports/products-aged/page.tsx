...
type ProductsAgedReport = {
  ...
  summary: {
    agedNinetyPlus: number;
    totalSkus: number;
    carryingCostCents: number;
  };
  items: AgedProductRow[];
};

export default function ProductsAgedReportPage() {
  const { branch: activeBranch, loading: branchLoading, error: branchError } = useActiveBranch();
  const [startDate, setStartDate] = useState(() => todayIsoDate());
  const [endDate, setEndDate] = useState(() => todayIsoDate());
  const [report, setReport] = useState<ProductsAgedReport | null>(null);
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reports/products-aged?${params.toString()}`);
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error ?? "No se pudo cargar el reporte de productos envejecidos");
      }

      const payload: ProductsAgedReport = await response.json();
      setReport(payload);
    } catch (err) {
      setReport(null);
  ...
  return (
    <ReportPageShell
      title="Products Aged"
      description="Identifica artículos con mayor tiempo en sala y su impacto financiero."
    >
      <div className="space-y-8">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Salud del inventario.</p>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <MetricCard
              label=">= 90 días"
              value={new Intl.NumberFormat("es-DO").format(report?.summary.agedNinetyPlus ?? 0)}
            />
            <MetricCard
              label="SKUs evaluados"
              value={new Intl.NumberFormat("es-DO").format(report?.summary.totalSkus ?? 0)}
            />
            <MetricCard label="Costo cargado" value={formatCurrencyFromCents(report?.summary.carryingCostCents ?? 0)} />
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Detalle por SKU</h2>
              <p className="text-sm text-slate-500">Incluye días en sala y últimas actualizaciones.</p>
            </div>
            ...
            <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
              <thead>
                <tr className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-800/60">
                  ...
                  <th className="px-4 py-3 text-right">Días en sala</th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {(report?.items ?? []).map((row) => (
                  <tr key={row.id}>
                    <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{row.productCode ?? "—"}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{row.productName ?? "—"}</td>
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{row.qtyOnHand}</td>
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">
                      {row.costCents == null ? "—" : formatCurrencyFromCents(row.costCents)}
                    </td>
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{row.daysOnHand ?? "—"}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{formatDateForDisplay(row.updatedAt)}</td>
                  </tr>
                ))}
                {(report?.items?.length ?? 0) === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-6 text-center text-sm text-slate-500">
                      No hay productos que cumplan con el filtro seleccionado.
                    </td>
                  </tr>
                ) : null}
              </tbody>
