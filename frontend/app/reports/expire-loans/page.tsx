...
import { formatCurrencyFromCents, formatDateForDisplay, isValidDateRange, todayIsoDate } from "@/lib/utils";
...
type ExpireLoansReport = {
  ...
  summary: {
    ...
    principalAtRiskCents: number;
    overdueLoans: number;
  };
  loans: ExpiringLoanRow[];
};

export default function ExpireLoansReportPage() {
  const { branch: activeBranch, loading: branchLoading, error: branchError } = useActiveBranch();
  const [startDate, setStartDate] = useState(() => todayIsoDate());
  const [endDate, setEndDate] = useState(() => todayIsoDate());
  const [report, setReport] = useState<ExpireLoansReport | null>(null);
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reports/loans/expiring?${params.toString()}`);
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error ?? "No se pudo cargar el reporte de préstamos por vencer");
      }

      const payload: ExpireLoansReport = await response.json();
      setReport(payload);
    } catch (err) {
      setReport(null);
  ...
  return (
    <ReportPageShell
      title="Expire Loans"
      description="Vigila los préstamos que están por vencer para activar recordatorios o renovaciones preventivas."
    >
      <div className="space-y-8">
        <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white/80 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/50 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-500">Rango de fechas (vence)</p>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Resumen de préstamos a vencer en el período.</p>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <MetricCard
              ...
              value={new Intl.NumberFormat("es-DO").format(report?.summary.expiringLoans ?? 0)}
            />
            <MetricCard
              label="Capital en riesgo"
              value={formatCurrencyFromCents(report?.summary.principalAtRiskCents ?? 0)}
            />
            <MetricCard
              label="Ya vencidos"
              value={new Intl.NumberFormat("es-DO").format(report?.summary.overdueLoans ?? 0)}
            />
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Préstamos por seguimiento</h2>
              <p className="text-sm text-slate-500">Incluye ticket, contacto y estado del vencimiento.</p>
            </div>
            ...
            <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
              <thead>
                <tr className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-800/60">
                  ...
                  <th className="px-4 py-3 text-right">Días para vencer</th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {(report?.loans ?? []).map((loan) => {
                  const contact = loan.contactPhone ?? loan.contactEmail ?? "—";

                  return (
                    <tr key={loan.id}>
                      <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{loan.ticketNumber ?? "—"}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{loan.customerName}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{contact}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{loan.status ?? "—"}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{formatDateForDisplay(loan.dueDate)}</td>
                      <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{loan.daysUntilDue ?? "—"}</td>
                      <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{loan.daysPastDue ?? "—"}</td>
                      <td className="px-4 py-3 text-right font-medium text-slate-900 dark:text-slate-100">
                        {formatCurrencyFromCents(loan.principalCents)}
                      </td>
                    </tr>
                  );
                })}
                {(report?.loans?.length ?? 0) === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-6 text-center text-sm text-slate-500">
                      No hay préstamos dentro del rango seleccionado.
                    </td>
                  </tr>
                ) : null}
              </tbody>
