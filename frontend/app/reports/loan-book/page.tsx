...
export default function LoanBookReportPage() {
  ...
  const [report, setReport] = useState<LoanBookReport | null>(null);
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reports/loan-book?${params.toString()}`);
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error ?? "No se pudo cargar el loan book");
      }

      const payload: LoanBookReport = await response.json();
      setReport(payload);
    } catch (err) {
      setReport(null);
  ...
  return (
    <ReportPageShell
      title="Loan Book"
      description="Obtén un panorama del portafolio: capital activo, riesgo y vencimientos próximos."
    >
      <div className="space-y-8">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Composición del libro de empeños.</p>
            </div>
            {loading ? <Loader2 className="h-5 w-5 animate-spin text-slate-400" /> : null}
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            <MetricCard label="Capital activo" value={formatCurrencyFromCents(report?.summary.activePrincipalCents ?? 0)} />
            ...
            <MetricCard
              ...
              value={new Intl.NumberFormat("es-DO").format(report?.summary.atRiskLoans ?? 0)}
            />
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Préstamos filtrados</h2>
              <p className="text-sm text-slate-500">Incluye días en cartera y estado.</p>
            </div>
            ...
            <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
              <thead>
                <tr className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-800/60">
                  ...
                  <th className="px-4 py-3 text-right">Días en libro</th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {(report?.loans ?? []).map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{row.daysOnBook ?? "—"}</td>
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{row.daysPastDue ?? "—"}</td>
                    <td className="px-4 py-3 text-right font-medium text-slate-900 dark:text-slate-100">
                      {formatCurrencyFromCents(row.principalCents)}
                    </td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{formatDateForDisplay(row.dueDate)}</td>
                  </tr>
                ))}
                {(report?.loans?.length ?? 0) === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-6 text-center text-sm text-slate-500">
                      No hay préstamos para el rango seleccionado.
                    </td>
                  </tr>
                ) : null}
              </tbody>
