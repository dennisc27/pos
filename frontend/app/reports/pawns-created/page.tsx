...
type PawnsCreatedReport = {
  ...
  loans: PawnCreatedRow[];
};

export default function PawnsCreatedReportPage() {
  const { branch: activeBranch, loading: branchLoading, error: branchError } = useActiveBranch();
  const [startDate, setStartDate] = useState(() => todayIsoDate());
  const [endDate, setEndDate] = useState(() => todayIsoDate());
  const [report, setReport] = useState<PawnsCreatedReport | null>(null);
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reports/pawns/created?${params.toString()}`);
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error ?? "No se pudo cargar el reporte de empeños creados");
      }

      const payload: PawnsCreatedReport = await response.json();
      setReport(payload);
    } catch (err) {
      setReport(null);
  ...
  return (
    <ReportPageShell
      title="Pawns Created"
      description="Revisa la originación de empeños, el volumen de capital y el ritmo diario por sucursal."
    >
      <div className="space-y-8">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <MetricCard label="Capital prestado" value={formatCurrencyFromCents(report?.summary.totalPrincipalCents ?? 0)} />
            <MetricCard
            ...
            <MetricCard
              ...
              value={formatCurrencyFromCents(report?.summary.averagePrincipalCents ?? 0)}
            />
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Empeños originados</h2>
              <p className="text-sm text-slate-500">Incluye ticket, cliente, estado y fechas clave.</p>
            </div>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {(report?.loans ?? []).map((loan) => (
                  <tr key={loan.id}>
                    ...
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{formatDateForDisplay(loan.createdAt)}</td>
                ...
                {(report?.loans?.length ?? 0) === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-6 text-center text-sm text-slate-500">
                      No hay empeños registrados para el rango seleccionado.
                    </td>
                  </tr>
                ) : null}
              </tbody>
