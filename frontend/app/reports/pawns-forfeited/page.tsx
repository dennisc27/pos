...
type PawnsForfeitedReport = {
  ...
  summary: {
    ...
    averagePrincipalCents: number;
  };
  forfeitures: PawnForfeitedRow[];
};

export default function PawnsForfeitedReportPage() {
  const { branch: activeBranch, loading: branchLoading, error: branchError } = useActiveBranch();
  const [startDate, setStartDate] = useState(() => todayIsoDate());
  const [endDate, setEndDate] = useState(() => todayIsoDate());
  const [report, setReport] = useState<PawnsForfeitedReport | null>(null);
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reports/pawns/forfeited?${params.toString()}`);
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error ?? "No se pudo cargar el reporte de empeños perdidos");
      }

      const payload: PawnsForfeitedReport = await response.json();
      setReport(payload);
    } catch (err) {
      setReport(null);
  ...
  return (
    <ReportPageShell
      ...
      description="Monitorea los empeños perdidos y el capital asociado que se convertirá en inventario."
    >
      <div className="space-y-8">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Impacto de los tickets perdidos.</p>
            </div>
            {loading ? <Loader2 className="h-5 w-5 animate-spin text-slate-400" /> : null}
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            <MetricCard label="Tickets perdidos" value={new Intl.NumberFormat("es-DO").format(report?.summary.totalForfeited ?? 0)} />
            <MetricCard label="Principal asociado" value={formatCurrencyFromCents(report?.summary.totalPrincipalCents ?? 0)} />
            <MetricCard label="Ticket promedio" value={formatCurrencyFromCents(report?.summary.averagePrincipalCents ?? 0)} />
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Detalle de empeños perdidos</h2>
              <p className="text-sm text-slate-500">Incluye código inventario resultante y fecha.</p>
            </div>
            ...
            <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
              <thead>
                <tr className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-800/60">
                  ...
                  <th className="px-4 py-3">Código inventario</th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {(report?.forfeitures ?? []).map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{row.productCode ?? "—"}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{row.productName ?? "—"}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{formatDateForDisplay(row.forfeitedAt)}</td>
                    <td className="px-4 py-3 text-right font-medium text-slate-900 dark:text-slate-100">
                      {formatCurrencyFromCents(row.principalCents)}
                    </td>
                  </tr>
                ))}
                {(report?.forfeitures?.length ?? 0) === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-6 text-center text-sm text-slate-500">
                      No hay empeños perdidos para el rango seleccionado.
                    </td>
                  </tr>
                ) : null}
              </tbody>
