...
export default function IncomeReportPage() {
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reports/incomes?${params.toString()}`);
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error ?? "No se pudo cargar el reporte de ingresos");
      }

      const payload: IncomeReportResponse = await response.json();
      setReport(payload);
    } catch (err) {
      setReport(null);
  ...
  return (
    <ReportPageShell title="Income Report" description="Review non-sales income streams and compare performance against targets.">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Entradas registradas.</p>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <MetricCard
              ...
              value={formatCurrencyFromCents(report?.summary.totalIncomeCents ?? 0)}
            />
            <MetricCard
              label="Movimientos"
            ...
            <MetricCard
              label="Promedio"
              value={formatCurrencyFromCents(report?.summary.averageIncomeCents ?? 0)}
            />
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Conceptos registrados.</p>
            </div>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {(report?.entries ?? []).map((row) => (
                  <tr key={row.id} className="bg-white/80 text-slate-700 dark:bg-slate-900/40 dark:text-slate-200">
                    <td className="px-4 py-3">{row.reason ?? "Ingreso"}</td>
