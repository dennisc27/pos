...
export default function PawnsOnSaleReportPage() {
  const { branch: activeBranch, loading: branchLoading, error: branchError } = useActiveBranch();
  const [startDate, setStartDate] = useState(() => todayIsoDate());
  const [endDate, setEndDate] = useState(() => todayIsoDate());
  const [report, setReport] = useState<PawnsOnSaleReport | null>(null);
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reports/pawns/on-sale?${params.toString()}`);
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error ?? "No se pudo cargar el reporte de empeños en venta");
      }

      const payload: PawnsOnSaleReport = await response.json();
      setReport(payload);
    } catch (err) {
      setReport(null);
  ...
  return (
    <ReportPageShell
      title="Pawns on Sale"
      description="Consulta la mercancía disponible proveniente de empeños perdidos y su valuación."
    >
      <div className="space-y-8">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Inventario disponible derivado de empeños.</p>
            </div>
            {loading ? <Loader2 className="h-5 w-5 animate-spin text-slate-400" /> : null}
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            <MetricCard
              label="Artículos en venta"
              value={new Intl.NumberFormat("es-DO").format(report?.summary.itemsOnSale ?? 0)}
            />
            <MetricCard label="Costo acumulado" value={formatCurrencyFromCents(report?.summary.totalCostCents ?? 0)} />
            <MetricCard label="Precio listado" value={formatCurrencyFromCents(report?.summary.totalRetailCents ?? 0)} />
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Inventario disponible</h2>
              <p className="text-sm text-slate-500">Incluye código, descripción, cantidades y valuación.</p>
            </div>
            ...
            <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
              <thead>
                <tr className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-800/60">
                  ...
                  <th className="px-4 py-3">Forfeitado</th>
                </tr>
              </thead>
                ...
                {(report?.items ?? []).map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="px-4 py-3 text-right font-medium text-slate-900 dark:text-slate-100">
                      {row.priceCents == null ? "—" : formatCurrencyFromCents(row.priceCents)}
                    </td>
                ...
                {(report?.items?.length ?? 0) === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-6 text-center text-sm text-slate-500">
                      No hay artículos disponibles para el rango seleccionado.
                    </td>
                  </tr>
                ) : null}
              </tbody>
