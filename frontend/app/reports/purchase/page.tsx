...
export default function PurchaseReportPage() {
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reports/purchases?${params.toString()}`);
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error ?? "No se pudo cargar el reporte de compras");
      }

      const payload: PurchaseReportResponse = await response.json();
      setReport(payload);
    } catch (err) {
      setReport(null);
      ...
      <div className="space-y-8">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <MetricCard
              ...
              value={formatCurrencyFromCents(report?.summary.totalBoughtCents ?? 0)}
            />
            <MetricCard
              label="Cantidad total"
              value={new Intl.NumberFormat("es-DO").format(report?.summary.totalQuantity ?? 0)}
            />
            <MetricCard
              label="Proveedores"
              value={new Intl.NumberFormat("es-DO").format(report?.summary.suppliersCount ?? 0)}
            />
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Órdenes recibidas</h2>
              <p className="text-sm text-slate-500">Detalle por proveedor y referencia.</p>
            </div>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {(report?.purchases ?? []).map((row) => (
                  <tr key={row.id} className="bg-white/80 text-slate-700 dark:bg-slate-900/40 dark:text-slate-200">
                    <td className="px-4 py-3 font-medium">{row.supplierName ?? "Sin proveedor"}</td>
                    <td className="px-4 py-3">{row.supplierInvoice ?? "—"}</td>
                    <td className="px-4 py-3">{row.referenceNo ?? "—"}</td>
                    <td className="px-4 py-3">{formatDateForDisplay(row.receivedAt ?? row.createdAt)}</td>
                    <td className="px-4 py-3 text-right">{formatCurrencyFromCents(row.totalCostCents)}</td>
                    <td className="px-4 py-3 text-right">{new Intl.NumberFormat("es-DO").format(row.totalQuantity)}</td>
                  </tr>
                ))}
                {report?.purchases?.length ? null : (
                  <tr>
