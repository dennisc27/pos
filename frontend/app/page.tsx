...
export default async function DashboardPage() {
  ...
  const performanceTrends = buildPerformanceSeries();
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-8 px-3 pb-10 sm:px-6 lg:px-8">
      ...
      <InsightCard
        ...
        description="Flip between daily and monthly views to compare transaction momentum."
        className="border-transparent bg-white/70 shadow-2xl ring-1 ring-slate-200/70 dark:border-slate-800/70 dark:bg-slate-900/80"
      >
        <PerformanceLineChart daily={performanceTrends.daily} monthly={performanceTrends.monthly} />
      </InsightCard>
...
function buildPerformanceSeries() {
  ...
  const daily = Array.from({ length: 30 }, (_, index) => {
    ...
    const sales = Math.max(4200 + Math.sin(index / 3) * 950 + index * 55, 600);
    const pawns = Math.max(38 + Math.cos(index / 4) * 11 + index * 0.6, 8);
    const layaways = Math.max(26 + Math.sin(index / 5) * 9 + index * 0.5, 6);

    return {
      label: date.toLocaleDateString("en-US", { month: "short", day: "numeric" }),
      sales: Math.round(sales),
      pawns: Math.round(pawns),
      layaways: Math.round(layaways)
    };
  });
  ...
  const monthly = Array.from({ length: 12 }, (_, index) => {
    ...
    const sales = Math.max(120000 + Math.sin(index / 2.3) * 18000 + index * 2500, 15000);
    const pawns = Math.max(620 + Math.cos(index / 2.8) * 130 + index * 12, 80);
    const layaways = Math.max(380 + Math.sin(index / 2.5) * 90 + index * 10, 50);
