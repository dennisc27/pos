    ...
    <main className="mx-auto max-w-6xl px-6 py-10 lg:px-10">
      <header className="mb-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-semibold text-slate-900">Past-due outreach</h1>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      <section className="mb-8 grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs uppercase tracking-wide text-slate-500">Balance total</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">
            {formatCurrencyFromCents(data?.summary?.totalDueCents ?? 0)}
          </p>
          <p className="mt-1 text-xs text-slate-500">Principal: {formatCurrencyFromCents(data?.summary?.totalPrincipalCents ?? 0)}</p>
          <p className="text-xs text-slate-500">Interés: {formatCurrencyFromCents(data?.summary?.totalInterestCents ?? 0)}</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs uppercase tracking-wide text-slate-500">Tickets atrasados</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{totalCount}</p>
          <p className="mt-1 text-xs text-slate-500">Generado el {formatDateTime(data?.generatedAt)}</p>
          <p className="text-xs text-slate-500">Corte {formatDate(data?.asOf)}</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs uppercase tracking-wide text-slate-500">Selección actual</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{selectedIds.length}</p>
          <p className="mt-1 text-xs text-slate-500">Se enviará {selectedIds.length > 0 ? "a los tickets marcados" : "al filtro completo"}</p>
        </div>
      </section>

      <section className="mb-6 flex flex-col gap-4 rounded-lg border border-slate-200 bg-white/70 p-4 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <form onSubmit={handleSearchSubmit} className="flex w-full max-w-md items-center gap-2">
            <input
              ...
              className="flex-1 rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
            <button
              type="submit"
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end">
          <label className="flex flex-col text-xs font-medium text-slate-600">
            ...
            <select
              ...
              className="mt-1 rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            >
          ...
          <label className="flex flex-1 flex-col text-xs font-medium text-slate-600 md:max-w-xs">
      ...
      <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-semibold text-slate-600">Totales</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loans.map((loan) => {
                ...
                return (
                  <tr key={loan.id} className={isSelected ? "bg-blue-50/40" : undefined}>
                    ...
                    <td className="px-4 py-3 align-top">
                      <div className="font-medium text-slate-900">{loan.ticketNumber}</div>
                      <div className="text-xs text-slate-500">{loan.branchName ?? "—"}</div>
                      <div className="mt-1 inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-600">
                        {loan.bucket} días
                      </div>
                    </td>
                    <td className="px-4 py-3 align-top">
                      <div className="font-medium text-slate-900">{customerName}</div>
                      <div className="text-xs text-slate-500">{loan.customer?.phone ?? "Sin teléfono"}</div>
                      {loan.customer?.email && (
                    ...
                    <td className="px-4 py-3 align-top">
                      <div className="font-medium text-slate-900">{formatDate(loan.dueDate)}</div>
                      <div className="text-xs text-slate-500">{loan.daysLate ?? 0} día(s) · Gracia {loan.graceDays}</div>
                      <div className="text-xs text-slate-500">Calendario: {loan.calendarDaysLate ?? 0} día(s)</div>
                    </td>
                    <td className="px-4 py-3 align-top">
                      <div className="font-medium text-slate-900">{formatCurrencyFromCents(loan.totalDueCents)}</div>
                      <div className="text-xs text-slate-500">Principal: {formatCurrencyFromCents(loan.outstandingPrincipalCents)}</div>
                      <div className="text-xs text-slate-500">Interés: {formatCurrencyFromCents(loan.outstandingInterestCents)}</div>
                    </td>
                    <td className="px-4 py-3 align-top">
                      <div className="text-xs text-slate-500">Último contacto: {lastOutreach}</div>
                      <div className="text-xs text-slate-500">Mensajes: {notifications}</div>
                      <div className="mt-1 text-xs text-slate-500">Teléfono envío: {loan.contactPhone ?? "—"}</div>
                    </td>
                  </tr>
                );
              })}
        ...
        {loading && (
          <div className="border-t border-slate-200 px-4 py-3 text-sm text-slate-500">Cargando información de tickets...</div>
        )}
        ...
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-slate-900/40 px-4 pb-8 pt-16">
          <div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-2xl">
            ...
            <textarea
              ...
              rows={5}
              className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
            ...
            {composer.success && (
              <div className="mt-3 rounded-md border border-green-200 bg-green-50 px-3 py-2 text-xs text-green-600">
                {composer.success}
              </div>
            )}
            ...
            <div className="mt-4">
              ...
              <ul className="max-h-40 space-y-2 overflow-y-auto rounded-md border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
                {composerPreview.length === 0 && <li>No hay vista previa disponible.</li>}
                {composerPreview.map((preview) => (
                  <li key={preview.loanId}>
                    <span className="font-medium text-slate-700">#{preview.loanId}:</span> {preview.message || "(mensaje vacío)"}
                  </li>
                ))}
              </ul>
            </div>
