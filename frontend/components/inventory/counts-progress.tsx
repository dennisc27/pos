...
const nextStatus: Record<CountSession["status"], CountSession["status"] | null> = {
...
export function CountsProgress({
  sessions,
  onAdvance,
  onOpenReconciliation,
}: {
  sessions: CountSession[];
  onAdvance?: (id: string, next: CountSession["status"] | null) => void;
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500 dark:text-slate-400">
                ...
                <div className="flex items-center gap-2">
                  <button
                    className="rounded-full border border-slate-300 px-3 py-1 text-slate-700 transition hover:border-slate-400 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:text-slate-300 dark:hover:border-slate-600"
                    onClick={() => onAdvance?.(session.id, next)}
                    disabled={!next}
                  >
                    {next ? `Mark ${next.replace("_", " ")}` : "Posted"}
                  </button>
                  <button
                    className="rounded-full border border-slate-300 px-3 py-1 text-slate-700 transition hover:border-slate-400 hover:text-slate-900 dark:border-slate-700 dark:text-slate-300 dark:hover:border-slate-600"
                    onClick={() => onOpenReconciliation?.(session.id)}
                  >
                    Reconciliation
                  </button>
                </div>
