...
export const ecomReturns = mysqlTable('ecom_returns', {
  id: bigint('id', { mode: 'number' }).primaryKey().autoincrement(),
  orderId: bigint('order_id', { mode: 'number' }).notNull(),
  status: mysqlEnum('status', ['requested', 'approved', 'received', 'refunded', 'denied']).default('requested'),
  reason: text('reason'),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow().onUpdateNow(),
});
...
export const ecomReturnItems = mysqlTable('ecom_return_items', {
  id: bigint('id', { mode: 'number' }).primaryKey().autoincrement(),
  returnId: bigint('return_id', { mode: 'number' }).notNull(),
  orderItemId: bigint('order_item_id', { mode: 'number' }).notNull(),
  condition: mysqlEnum('condition', ['new', 'used', 'damaged']).default('new'),
  restock: boolean('restock').default(true),
  createdAt: timestamp('created_at').defaultNow(),
});
