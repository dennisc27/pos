...
ordersRouter.post('/', async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(itemsInput) || itemsInput.length === 0) {
      return res.status(400).json({
        error: 'InvalidItems',
        message: 'The order must include at least one line item.',
      });
    }

    const normalizedInputItems = itemsInput.map((item, index) => {
    ...
    const totalBigInt = subtotalBigInt + BigInt(taxCents);
